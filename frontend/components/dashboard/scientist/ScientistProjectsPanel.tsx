      ...
      {projects.map((project) => (
        <Card key={project.id} variant="outlined" className="hover:shadow-md transition-shadow">
          ...
          <CardFooter className="flex flex-col items-stretch gap-3 sm:flex-row">
            <Link href={`/projects/${project.id}`} className="w-full flex-1 min-w-0">
              <Button variant="outline" className="w-full text-sm">Открыть</Button>
            </Link>
            <Button variant="outline" className="w-full flex-1 min-w-0 text-sm" onClick={() => handleEditProject(project)}>
          ...
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 sm:p-6">
              <div className="mb-6 flex items-center justify-between gap-3">
                <h2 className="text-xl font-bold text-foreground sm:text-2xl">Редактирование проекта</h2>
                <button
